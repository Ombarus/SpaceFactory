...
func set_attrib(modified_attributes : Dictionary, path : String, val) -> void:
	...
	if typeof(val) == TYPE_VECTOR2:
		val = {"__class":"Vector2", "value": var_to_str(val)}
	elif typeof(val) == TYPE_VECTOR3:
		val = {"__class":"Vector3", "value": var_to_str(val)}
	elif typeof(val) == TYPE_TRANSFORM3D:
		val = {"__class":"Transform3D", "value": var_to_str(val)}
	elif not typeof(val) in [TYPE_NIL, TYPE_BOOL, TYPE_INT, TYPE_FLOAT, TYPE_STRING, TYPE_DICTIONARY, TYPE_ARRAY]:
...
func mytr2(text : String, fmt:={}) -> String:
	...
	if OS.is_debug_build() and result == text and TranslationServer.get_locale() != "en":
		print("====> [TR] : No %s position for %s" % [TranslationServer.get_locale(), text])
